...
{
  FILE *entree=fopen("fichier_inter.csv", "r");
  if(entree==NULL)
	{
		exit(1);
	}
  FILE *sorti=fopen("fichier_sorti", "rw");
	if(sorti==NULL)
	{
		exit(2);
	}
  float temp;
  int id;
	pStation p=malloc(sizeof(Arbre));
	if(p==NULL)
	{
		exit(2);
  }
  while(fic!=EOF)
	{
		fscanf(fic, %d %f,&id,&temp);
    p=inserer_noeud(p,id,temp);
    return p;
    
  }
  fclose(entree);
  parcoursInfixe(fic_sort,p);
  fclose(sorti);
}
